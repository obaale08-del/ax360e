...
#ifndef _WIN32
static uint64_t _fd_getFileSize(int fd)
{
...
static bool _fd_readBytes(int fd, uint64_t offset, void* buffer, uint32_t size)
{
	...
	while (remaining > 0)
	{
		ssize_t bytesRead = read(fd, bufPtr, remaining);
		if (bytesRead <= 0)
			return false;
		bufPtr += bytesRead;
...
#ifndef _WIN32
ZArchiveReader* ZArchiveReader::OpenFromFile(int fd)
{
	uint64_t fileSize = _fd_getFileSize(fd);
	if (fileSize <= sizeof(_ZARCHIVE::Footer))
		return nullptr;
	// read footer
	_ZARCHIVE::Footer footer;
	if (!_fd_readBytes(fd, fileSize - sizeof(_ZARCHIVE::Footer), &footer, sizeof(_ZARCHIVE::Footer)))
		return nullptr;
	_ZARCHIVE::Footer::Deserialize(&footer, &footer);
	// validate footer
	if (footer.magic != _ZARCHIVE::Footer::kMagic)
	...
	std::vector<_ZARCHIVE::CompressionOffsetRecord> offsetRecords;
	offsetRecords.resize(_getValidElementCount(footer.sectionOffsetRecords.size, sizeof(_ZARCHIVE::CompressionOffsetRecord)));
	if (offsetRecords.empty() || !_fd_readBytes(fd, footer.sectionOffsetRecords.offset, offsetRecords.data(), (uint32_t)(offsetRecords.size() * sizeof(_ZARCHIVE::CompressionOffsetRecord))))
		return nullptr;
	_ZARCHIVE::CompressionOffsetRecord::Deserialize(offsetRecords.data(), offsetRecords.size(), offsetRecords.data());
	// read name table
	std::vector<uint8_t> nameTable;
	nameTable.resize(footer.sectionNames.size);
	if (!_fd_readBytes(fd, footer.sectionNames.offset, nameTable.data(), (uint32_t)(nameTable.size() * sizeof(uint8_t))))
		return nullptr;
	// read file tree
	std::vector<_ZARCHIVE::FileDirectoryEntry> fileTree;
	fileTree.resize(_getValidElementCount(footer.sectionFileTree.size, sizeof(_ZARCHIVE::FileDirectoryEntry)));
	if (fileTree.empty() || !_fd_readBytes(fd, footer.sectionFileTree.offset, fileTree.data(), (uint32_t)(fileTree.size() * sizeof(_ZARCHIVE::FileDirectoryEntry))))
		return nullptr;
	_ZARCHIVE::FileDirectoryEntry::Deserialize(fileTree.data(), fileTree.size(), fileTree.data());
	// verify file tree
	if (fileTree[0].IsFile())
	...
	ZArchiveReader* cfs = new ZArchiveReader(fd, std::move(offsetRecords), std::move(nameTable), std::move(fileTree), footer.sectionCompressedData.offset, footer.sectionCompressedData.size);
	return cfs;
}
#endif
